...
├── app.py                          # Streamlit demo app (loads the trained model)
...
These three files are required by `app.py` — run the notebook once first if they don't exist yet.

## Demo App

A simple Streamlit web app is included so anyone can try the model without opening the notebook:

```bash
streamlit run app.py
```

This opens a form in the browser where you enter a patient's clinical and lab values, and the app returns the predicted cirrhosis stage along with the model's confidence for each class.

## Possible Next Steps

- SHAP-based interpretability for the best model
- Stratified k-fold cross-validation instead of a single train/test split
- Extend `app.py` to also support the other trained models (LR/SVM/KNN need the saved `scaler.pkl`)
